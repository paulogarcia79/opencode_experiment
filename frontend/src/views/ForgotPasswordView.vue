    ...
    <div class="relative w-full max-w-md mx-4">
      ...
      <div class="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm p-8 shadow-2xl shadow-black/20">
        ...
        <div v-if="error" class="mb-6 border border-red-500/20 bg-red-500/10 rounded-lg p-4 flex items-start gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-red-300">Request Failed</p>
            <p class="text-sm text-red-400/80 mt-0.5">{{ error }}</p>
          </div>
        </div>

        <!-- Success State -->
        <div v-if="success" class="mb-6 border border-green-500/20 bg-green-500/10 rounded-lg p-4 flex items-start gap-3">
          <svg class="h-5 w-5 text-green-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <div>
            <p class="text-sm font-medium text-green-300">Check your email</p>
            <p class="text-sm text-green-400/80 mt-0.5">If an account exists with that email, we've sent a password reset link.</p>
          </div>
        </div>
