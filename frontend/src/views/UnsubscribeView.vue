...
import { unsubscribeFromNewsletter } from '@/composables/useApi'
...
onMounted(async () => {
  ...
  try {
    ...
    status.value = 'error'
    errorMessage.value = err instanceof Error ? err.message : 'Failed to unsubscribe.'
  }
})
</script>
...
<template>
  <div class="min-h-screen bg-surface-950 flex items-center justify-center relative overflow-hidden">
    ...
    <div class="absolute top-1/4 left-1/4 w-72 h-72 bg-slate-600/10 rounded-full blur-3xl" />
    ...
    <div class="relative w-full max-w-md mx-4 text-center">
      <div v-if="status === 'loading'" class="flex flex-col items-center">
        <div class="w-12 h-12 border-4 border-slate-500/20 border-t-slate-500 rounded-full animate-spin mb-6"></div>
        <h1 class="text-xl font-display font-bold text-white">Processing your request...</h1>
      </div>

      <div v-else-if="status === 'success'">
        <div class="w-16 h-16 rounded-2xl bg-slate-500/10 border border-slate-500/20 flex items-center justify-center mx-auto mb-6">
          <svg class="h-8 w-8 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h1 class="text-2xl font-display font-bold text-white">Unsubscribed</h1>
        <p class="mt-3 text-slate-400 leading-relaxed">
          You have been removed from the newsletter. We're sorry to see you go.
        </p>
      </div>
      ...
      <div v-else-if="status === 'error'">
        ...
        <h1 class="text-2xl font-display font-bold text-white">Unsubscribe Failed</h1>
        <p class="mt-3 text-slate-400 leading-relaxed">
          {{ errorMessage }}
        </p>
      </div>
